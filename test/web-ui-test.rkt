...
(module+ test
  (test-case "museum catalog has exhibit cards instead of platform tabs"
    ...
    (check-true (regexp-match? #rx"id=\"filter-desktop\"" html))
    (check-true (regexp-match? #rx"id=\"filter-mobile\"" html))
    (check-true (regexp-match? #rx"id=\"filter-desktop\"[^>]+checked=\"checked\"" html))
    (check-true (regexp-match? #rx"id=\"filter-mobile\"[^>]+checked=\"checked\"" html))
    (check-true (regexp-match? #rx"for=\"filter-desktop\"" html))
    (check-true (regexp-match? #rx"for=\"filter-mobile\"" html))
    (check-true (regexp-match? #rx"data-platforms=\"desktop\"" html))
    (check-true (regexp-match? #rx"data-platforms=\"mobile\"" html))
    (check-true (regexp-match? #rx"data-card-platform=\"desktop\"" html))
    (check-true (regexp-match? #rx"data-card-platform=\"mobile\"" html))
    (check-true (regexp-match? #rx"data-href=\"/exhibits/flypy\"" html))
    (check-equal? (match-count #rx"data-href=\"/exhibits/flypy\"" html) 2)
    ...
    (check-true (regexp-match? #rx"type=\"checkbox\"" html))
  ...
  (test-case "catalog supports Traditional Chinese copy"
    ...
    (check-true (regexp-match? #rx"桌面" html))
    (check-true (regexp-match? #rx"移動" html))
